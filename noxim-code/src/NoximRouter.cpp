    ...
    int >NoximRouter::routingFunction(const NoximRouteData & route_data)
{
    ...
    case ROUTING_CUSTOM:				//custom
	return routingCustom(position, dst_coord);	//custom
...
int d_M(int x1, int x2, int y1, int y2);    //
int d_M(int x1, int x2, int y1, int y2)     //function to calculate manhattan distance between two nodes aka hop count 
{	  				//
...
///*
vector < int >NoximRouter::routingCustom(const NoximCoord & current,
					    const NoximCoord & destination)
{
    //cout<< "Yea!";
    vector < int >directions;
    ...
    int d_without_link, d_with_link, use_link;
    d_without_link = d_M(current.x, destination.x, current.y, destination.y);
    d_with_link    = tile_is_4 == 1 ? (1 + d_M(2, destination.x, 1, destination.y) ) : 
